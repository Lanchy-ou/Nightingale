...
- 只有具备 strict exact source 的 system-authored AI Summary Highlight 成功通过 status CAS 后才写 feedback；Span 必须有显式、in-bounds、非空 offset，AI Summary 自引与 Summary pointer/source 不一致均拒绝。no-op、409、patient/admin、非 AI row 均不训练。Staff feedback 不设置 `clinician_confirmed`。
...
- `backend/tests/test_self_learning_importance.py`：18 passed；覆盖 current pin vs future learning、accept/pin/reject、same-base control/final ordering、clinic isolation、staff authority、patient/admin、latest-only、no-op/CAS loser、caps、全部 hard protections、metadata-only、strict exact provenance、missing-offset rejection、AI Summary self-citation rejection、read path、tiebreak、UI explanation、SQLite/SQLCipher migration。
...
- Regression：backend full `395 passed`；frontend production build PASS；D3 corpus validation/runtime hard gates PASS；D4 frozen Copilot eval PASS；security/integration `17 passed`；SQLCipher migration/storage、secret scan、`pip check`、Caddy validate 与 `git diff --check` PASS。
