...
- **F_A2 implementation evidence (2026-09-02)**: deterministic scope and review-closure hardening are implemented and verified with limits. Candidate-level Nurse decisions, Staff Note-backed corrections, clinician/Nurse-owned downstream Care Tasks, explicit Glance factor explanations, idempotent escalation and RBAC passed the full 544-test collection (542 passed / 2 existing local-ASR-input skips), frontend production build and a normal Session patient -> Nurse -> clinician browser journey. FHIR R5 resource names are conceptual classification references only; no FHIR conformance is claimed. The final reason-code stability matrix is deferred until the full F task set is complete. F_A1 still has no authorization to train a model.
