...
- **Canonical A task cards**: `Task_Card/F_A2_Importance_Semantics_Task_Card.md`, `F_A1_Self_Learning_Trust_Task_Card.md`, `F_A3_Clinic_Isolation_Defense_Task_Card.md`, `F_A4_Log_Privacy_Boundary_Task_Card.md`, and `F_A5_Provider_Total_Timeout_Task_Card.md`. F_A2's deterministic first/mid contract was owner-approved on 2026-09-01 and executes before F_A1. F_A1 remains blocked on F_A2 evidence and may not train a model. Neither card may invent medical weights, thresholds, must-surface cases, or clinical ground truth.
- **F_A2 approved v1**: `staff` is the Nurse role; each submitted Check-in creates one exact-source Nurse review workflow; exact-source AI suggestions may route an unverified priority review; production review/escalation is 720 minutes and test/dev may use 5 minutes with injected time; clinicians record outcome plus time-sensitivity labels. No real-time alert, external delivery or clinical-validity claim is added.
- **F_A2 implementation evidence (2026-09-02)**: deterministic scope and review-closure hardening are implemented and verified with limits. Candidate-level Nurse decisions, Staff Note-backed corrections, clinician/Nurse-owned downstream Care Tasks, explicit Glance factor explanations, idempotent escalation and RBAC passed regression plus a normal Session patient -> Nurse -> clinician browser journey. The final reason-code matrix is complete: mock + server validator `32/32`, deterministic fallback `28/28` language cases with no priority code, exact mismatch `4/4` dropped, live Provider `NOT_RUN`. The first run exposed 19 false-positive mock routes; `app/priority_routing.py` now fails uncertain/non-current/non-self/mismatched codes closed to routine Nurse review without deleting the patient report. FHIR R5 names remain conceptual only; clinical validity and F_A1 model training remain unauthorized/not established.
- **F_A1 implementation evidence (2026-09-02)**: formal Glance is A2 `base_only`; status controls no longer train. Immutable RankingRun/Decision records cover surfaced, unsurfaced and excluded candidates; role-bounded LearningSignal, Coverage Review, replay, freeze and Shadow rollback are implemented. Full collection is 556 tests (554 passed / 2 existing local-ASR-input skips), frontend build is 62 modules, and Nurse -> clinician -> Admin normal-Session acceptance passed. No model was trained and learning-enabled serving remains rejected.
- **F_A3 implementation evidence (2026-09-02)**: owner-approved one-query scoped loaders now protect patient-bound direct-object routes independently of `authorize_scope`; SQLite/SQLCipher foreign keys, metadata-only ownership preflight, ownership-only triggers, scope indexes and an AST bypass gate enforce the bounded second layer. Fault-injected reads/writes, inconsistent Event/Task/Highlight/Ranking/Learning links, A1 single-transaction batching and two real Cookie Sessions passed. Full backend is 579 collected / 577 passed / 2 existing local-ASR-input skips; frontend build remains 62 modules. This is `IMPLEMENTED_WITH_LIMITS`: no PostgreSQL RLS or production multi-tenant certification, and visual browser acceptance is `NOT_RUN` because the browser runtime failed before tab control.
- **F_A4 implementation evidence (2026-09-02, rev 3 after owner review)**: owner-approved v1 log/operational privacy boundary is `IMPLEMENTED_WITH_LIMITS`. `backend/app/operational_logging.py` adds allowlisted structured stderr logging with **per-field value validators** (invalid/free-text values dropped, not just unknown keys), accepts route templates only from the matched Starlette route object, adds a deterministic PHI scrubber, keeps `emit_log` never-raising, and suppresses a duplicate uvicorn traceback only when `exc_info` carries the exact exception marked by the sanitizing application handler; unmarked server errors remain visible. All four existing log events migrated. `RequestIdMiddleware` (`security.py`) is pure ASGI and never trusts a client `X-Request-ID`. ASR `failure_reason` is a frozen fixed-code set enforced at both `ASRResult` (`voice/contracts.py`) and `fail_capture` (`voice/state_machine.py`); Voice audit `details` records `error_code` (`api/voice.py:567`). Seed output no longer prints the DB URL/path. Committed `deploy/Caddyfile` has `admin off` plus a global error-log `discard` only — access logging is NOT ENABLED (no per-site `log` blocks), verified by executable `caddy adapt` assertions, at the cost of PARTIAL edge fault observability (502/TLS/cert). Crash monitoring and Provider retention remain `NOT_ESTABLISHED` (no SDK; redaction is not a deletion guarantee); clinical `AuditLog` retention is `NEEDS_OWNER_POLICY`/`NOT_IMPLEMENTED`; host-side stderr capture/retention is `NOT_ESTABLISHED`. Real-process tests cover seed stdout and a real `uvicorn --no-access-log` 500/422 stderr journey. Full backend 601 collected / 599 passed / 2 existing local-ASR-input skips; frontend build 62 modules; `git diff --check`, secret scan and Caddy validate pass. Evidence: `docs/fa4_log_privacy_evidence_2026-09-02.md`.
...
- **F_A5 implementation evidence (2026-09-02)**: owner-approved explicit Provider total timeout is `IMPLEMENTED_WITH_LIMITS`. `backend/app/llm_client.py` adds fixed constants `PROVIDER_TOTAL_DEADLINE_SECONDS=30.0` and `PROVIDER_CONNECT/Pool/WRITE/READ = 5/5/10/30` (no env/UI config; tests inject short deadlines by monkeypatching the module constant), a `ProviderTimeoutError(LLMError)`, and `_run_async_with_total_deadline` which runs the Provider call through `asyncio.run(asyncio.wait_for(...))` so a timeout CANCELS the underlying async request rather than leaving a blocking sync thread. `DeepSeekAdapter._messages_create_async` lazily imports the SDK-compatible `anthropic.Timeout`, uses `AsyncAnthropic(..., timeout=Timeout(connect=5,pool=5,write=10,read=30), max_retries=0)`, and maps `APITimeoutError` to `ProviderTimeoutError`; all four entries (`verify_connection`, `summarize`, `copilot`, `_bounded_json` turn/summary) route through the single `_run_messages_create` choke point. The four flows explicitly catch `ProviderTimeoutError`: Consult (`ai_pipeline.py`) and Check-in (`checkins.py`) → `fallback_reason="provider_timeout"` + deterministic fallback with raw preserved; Copilot (`copilot.py`) → `status="unavailable"`, empty claims, no draft; key verification (`api/system_settings.py`) → 503 with no key saved. Check-in projects the fixed fallback reason and shows patient-safe saved-source + timeout + fallback copy. Timeout logs the fixed `provider_timeout` error code (added to the F_A4 allowlist), not a Python class name. Seven failure-first tests (`tests/test_fa5_provider_timeout.py`) include a real `AsyncAnthropic` request to a local never-responding HTTP server and prove deadline return plus socket disconnect, as well as raw preservation and retry idempotency with no duplicate Event/Artifact. Final full backend 609 passed / 2 existing local-ASR-input skips; frontend build 62 modules; `git diff --check` and secret scan pass. A real server-session patient login opened the independent Check-in page with zero browser warning/error. Remaining limit: no live DeepSeek key/external service was used, so a real external Provider timeout and its timeout-specific browser state were NOT observed; that copy is covered by API/frontend contract tests. Evidence: `docs/fa5_provider_total_timeout_evidence_2026-09-02.md`.
- **F1/A final closeout (2026-09-02)**: `F1_A_COMPLETE_WITH_LIMITS`. D safeguards survived final regression; A/D targeted `124 passed`; full backend `611 collected / 609 passed / 2 existing skips`; frontend build `62` modules; Patient/Nurse/Clinician/Admin server-session browser journeys had zero warning/error. The final 16-scenario ledger, reason-code matrix, repository state and external-delivery boundary are in `docs/f1_a_closeout_2026-09-02.md`. B/C remain unimplemented/limited as recorded; no live Provider, training, commit, push or external delivery was performed.
