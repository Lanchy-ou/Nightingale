...
## 25. D2 Implementation Status（2026-08-27）
...
- **Glance write path**: Task create/transition calls `recompute_task_highlights`; only a real unresolved Task with matching Event plus exact Artifact/Span sets `feature_flags.unresolved_task=true`, then `importance_score` is recomputed with frozen weights. `reported_done` stays unresolved until clinic `completed`; terminal completion/cancellation clears the weight. Glance reads remain precomputed and LLM-free.
...
- **Verification**: five new D2 test files add 23 tests; backend **245 passed**, frontend TypeScript/Vite production build passed. Local browser QA verified patient report done → waiting clinic verification → clinician verify complete → Glance refresh, staff shared shell, and zero console warnings/errors.
