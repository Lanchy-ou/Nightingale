...
- **Verification**: backend **294 passed**; corpus validation and deterministic runtime runner exit 0; frontend production build passed. Review re-verification added canonical-boundary regressions (`tests/test_transcript_preview_contract.py`) locking that preview source ranges can never enter the canonical Transcript, plus `CORPUS_VALIDATION_PASS` and the unified mock/deepseek provider wording. Browser QA covered ACCEPT/NEEDS_REVIEW/REJECT, resolve/split/merge, confirm → fallback → exact source, patient-switch isolation, and zero console warnings/errors.
