...
- **Glance write path**: the Task↔Glance mapping is ALWAYS explicit: `Highlight.task_id` points at exactly one Task and each Task is represented by at most one Highlight (`link_task_highlight`). Task creation adopts an existing UNOWNED task-type Highlight only on EXACT patient/event/source_artifact/source_span equality, otherwise it creates a dedicated task Highlight row — an Event-only Task therefore never flags unrelated Highlights in the same Event, and a new unresolved Task can always surface in Glance through its own row. `recompute_task_highlights` updates only `Highlight.task_id == task.task_id`; terminal completion/cancellation deterministically clears the unresolved weight on that Highlight alone (autoflush is disabled project-wide, so `create_task` flushes after linking). Glance reads remain precomputed and LLM-free.
...
- **Verification**: five new D2 test files plus review regressions (explicit-mapping isolation, event-only Glance entry, cancel clears, adoption-not-duplication, malformed-span 422-not-500, seed mapping lock); backend **254 passed**, frontend TypeScript/Vite production build passed. Local browser QA verified patient report done → waiting clinic verification → clinician verify complete → Glance refresh, staff shared shell, and zero console warnings/errors.
